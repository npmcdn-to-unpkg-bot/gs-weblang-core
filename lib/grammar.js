function Grammar(Parser, lexer, names, behaviour) {
    ...
    g.stmt(n.FUNCTION, function () {
        ...
        var body = bodyStatement();
        var ret = !body || body[body.length - 1];
        if (ret.alias !== "return" || !ret.expression) {
            g.error(token, "Las funciones deben terminar con un " + n.RETURN + " pero se encontró " + (ret.value || "vacío"));
        }
        g.scope.pop();
        var declaration = b.functionDeclaration(name, parameters, body, ret.expression);
